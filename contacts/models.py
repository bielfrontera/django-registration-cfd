...
class Course(models.Model):
    ...
    url_moodle = models.CharField(_('url_moodle'), max_length=200)

    date_added = models.DateTimeField(_('date added'), auto_now_add=True)
    user_add = models.ForeignKey(User, blank=True, null=True, related_name='course-add')
    date_modified = models.DateTimeField(_('date modified'), auto_now=True)
    user_modify = models.ForeignKey(User, blank=True, null=True, related_name='course-modified')
    ...
    def get_absolute_url(self):
        return ('contacts_course_detail', None, {
            'code': self.code,
        })

    @permalink
    def get_update_url(self):
        return ('contacts_course_update', None, {
            'code': self.code,
        })

    @permalink
    def get_delete_url(self):
        return ('contacts_course_delete', None, {
            'code': self.code,
        })

    @permalink
    def get_copy_url(self):
        return ('contacts_course_copy', None, {
            'code': self.code,
        })
...
class Person(models.Model):
    ...
    def get_courses_display(self):
        return ', '.join([a.code for a in self.courses.all()])
...
class MailTemplate(models.Model):
    ...
    date_added = models.DateTimeField(_('date added'), auto_now_add=True)
    user_add = models.ForeignKey(User, blank=True, null=True, related_name='mailtemplate-add')
    date_modified = models.DateTimeField(_('date modified'), auto_now=True)
    user_modify = models.ForeignKey(User, blank=True, null=True, related_name='mailtemplate-modified')
